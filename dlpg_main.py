...
from model.utils.utils import np2torch, torch2np, scale_down, scale_down_list, get_runname, scaling, get_diversity
...
def main(args):
    ...
    SAVE_WEIGHT_PATH = "./weights/{}/dlpg/".format(args.env_name)+args.runname
    ...
    for epoch in range(args.total_epochs):
        ...
        if (np.random.rand()>exploration_rate): # Exploitation
            ...
            if args.env_name=='reach' or args.env_name=='place':
                xs,ys,zs,_ = dlpg.exploit_3d(c=np2torch(c, device=args.device).reshape(-1,args.cdim), z=z)
                ...
                anchor = dlpg.grp.get_anchors_from_traj_3d(xs=xs, ys=ys, zs=zs, n_anchor=10)
            ...
            max_reward = -10
            if args.env_name=='reach' or args.env_name=='place':
                xs_lst, ys_lst, zs_lst = dlpg.random_explore_3d(n_sample=10, scenario=env.scenario)
            ...
            with torch.no_grad():
                ...
                for i in range(10):
                    ...
                    if args.env_name=='reach' or args.env_name=='place':
                        ...
                        trajectories.append(np.array([eval_xs, eval_ys, eval_zs]).reshape(3,10))
                    else:
                        xs, ys, _ = dlpg.exploit(c=np2torch(c, device=args.device).reshape(-1,args.cdim), z=z)
                        # Execute the trajectory
...
if __name__=="__main__":
    ...
    parser.add_argument('--xdim', type=int, default=30) 
