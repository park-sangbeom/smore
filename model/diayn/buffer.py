...
BenchmarkTransition = namedtuple('BenchmarkTransition', ('state', 'z', 'done', 'action', 'next_state', 'reward'))
...
class Benchmark_Memory:
    ...
    def add(self, *transition):
        self.buffer.append(BenchmarkTransition(*transition))
        if len(self.buffer) > self.buffer_size:
            self.buffer.pop(0)
        assert len(self.buffer) <= self.buffer_size
