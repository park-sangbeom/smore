...
import sys 
sys.path.append('..')
from model.diayn.buffer import Memory, Transition, Benchmark_Memory, BenchmarkTransition
from model.diayn.actor_critic import PolicyNetwork, QvalueNetwork, ValueNetwork, Discriminator
...
class SACAgent:
    def __init__(self,
        ...
        self.benchmark_memory = Benchmark_Memory(self.config["mem_size"], self.config["seed"])
    ...
    def benchmark_store(self, state, z, done, action, next_state, reward):
        state = from_numpy(state).float().to("cpu")
        z = torch.ByteTensor([z]).to("cpu")
        done = torch.BoolTensor([done]).to("cpu")
        action = torch.Tensor([action]).to("cpu")
        ...
        self.benchmark_memory.add(state, z, done, action, next_state, reward)
    ...
    def benchmark_unpack(self, batch):
        batch = BenchmarkTransition(*zip(*batch))
    ...
    def benchmark_train(self):
        if len(self.benchmark_memory) < self.batch_size:
            return None
        else:
            batch = self.benchmark_memory.sample(self.batch_size)
            states, zs, dones, actions, next_states, rewards = self.benchmark_unpack(batch)
            # Calculating the value target
            reparam_actions, log_probs = self.policy_network.sample_or_likelihood(states)
            q1 = self.q_value_network1(states, reparam_actions)
            q2 = self.q_value_network2(states, reparam_actions)
